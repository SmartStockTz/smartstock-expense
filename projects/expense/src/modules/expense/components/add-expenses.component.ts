      ...
      <form *ngIf="storeOutFormGroup" [formGroup]="storeOutFormGroup">
        ...
        <mat-card>
          <table *ngIf="(deviceState.isSmallScreen | async)===false" mat-table [dataSource]="storeOutDataSource">
            <ng-container cdkColumnDef="name">
              ...
              <td mat-cell *cdkCellDef="let element">{{element.item.name}}</td>
            ...
            <ng-container cdkColumnDef="amount">
              ...
              <td mat-cell *cdkCellDef="let element">
                <input [id]="element.item.id" style="min-width: 150px" [min]="1" [max]="element.amount"
                       class="quantity-input"
                       (change)="updateExpenseAmount(element, $event)" type="number" min="1"
            ...
            <div *ngFor="let item of storeOutDataSource.connect() | async">
              <mat-list-item>
                ...
                <div style="margin-bottom: 8px" matLine>
                  <input [id]="item.item.id" style="min-width: 150px" [min]="1" [max]="item.amount"
                         class="quantity-input"
                         (change)="updateExpenseAmount(item, $event)" type="number" min="1"
                         [value]="item.amount">
                </div>
        ...
        <div style="margin-bottom: 16px; display: flex; flex-direction: row; flex-wrap: wrap">
          <button style="margin-top: 24px"
                  ...
                  (click)="addExpenseItemToTable($event)" mat-flat-button color="primary">
            <mat-icon matSuffix>add</mat-icon>
            Choose Item
          </button>
          <div style="width: 16px; height: 16px"></div>
          <button style="margin-top: 24px" (click)="addExpense()"
...
export class AddExpensesComponent implements OnInit {
  ...
  addExpenseItemToTable($event: MouseEvent): any {
    ...
    if (isSmallScreen) {
      this.sheet.open(AddExpenseSearchBottomSheetComponent).afterDismissed().subscribe(value => {
        if (value && value.name) {
      ...
      this.dialog.open(AddExpenseSearchDialogComponent).afterClosed().subscribe(value => {
        if (value && value.name) {
          this.selectedProducts.push({
            amount: 0,
            item: value
  ...
  removeItem($event: MouseEvent, element: ExpenseModel): void {
    $event.preventDefault();
    this.selectedProducts = this.selectedProducts.filter(x => x.item.id !== element.item.id);
    this.storeOutDataSource = new MatTableDataSource<any>(this.selectedProducts);
    this.updateAmount();
  }
  ...
  async addExpense(): Promise<void> {
    if (this.storeOutDataSource.data.length === 0) {
      ...
      this.stockService.addExpenses(saveStoreOutData)
        .then(_ => {
