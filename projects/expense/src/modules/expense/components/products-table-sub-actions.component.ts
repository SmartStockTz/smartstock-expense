...
@Component({
  ...
  template: `
      <mat-card-subtitle>
          <button [disabled]="(stockState.isDeleteExpenseItems | async)===true" mat-stroked-button mat-button
                  color="primary" class="stockbtn"
                  (click)="deleteMany()">
              <mat-icon>cached</mat-icon>
              Delete
              <mat-progress-spinner *ngIf="(stockState.isDeleteExpenseItems | async)===true"
                                    mode="indeterminate"
                                    color="primary"
                                    diameter="20px"
