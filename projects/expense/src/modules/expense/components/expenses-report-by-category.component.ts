...
export class ExpensesReportByCategoryComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit(): Promise<any> {
    this.storeState.expenseReportByCategory.pipe(takeUntil(this.destroy)).subscribe(value => {
      if (value) {
        this.initiateGraph(value);
      }
    });
    this.storeState.expenseFrequencyGroupByCategory(
