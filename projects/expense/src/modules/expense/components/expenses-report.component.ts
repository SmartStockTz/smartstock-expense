...
export class ExpensesReportComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  storeColumns = ['tag', 'amount', 'action'];
  ...
  ngOnInit(): void {
    this.storeState.expenseReportByTagWithTrack.pipe(takeUntil(this.destroy)).subscribe(value => {
      if (value) {
        this.stockDatasource = new MatTableDataSource(value);
      }
    });
    this.storeState
      .expenseFrequencyGroupByTagWithTracking(this.storeState.reportStartDate.value, this.storeState.reportEndDate.value)
      .catch(console.log);
  }
  ...
  openStoreDetails(store: any): void {
    this.bottomSheet.open(ExpenseReportDetailsComponent, {
      data: store,
