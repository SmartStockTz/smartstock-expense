...
@Component({
  ...
  template: `
    <mat-card-title class="d-flex flex-row">
      <button routerLink="/expense/item/in" color="primary" class="ft-button" mat-flat-button>
        Add Item
      </button>
      <span class="toolbar-spacer"></span>
      <span style="width: 8px; height: 8px"></span>
      <button (click)="stockState.getStoresFromRemote()"
              [disabled]="(stockState.isFetchExpenseItems | async) === true"
              matTooltip="Reload from server"
              color="primary"
              class="ft-button" mat-flat-button>
        <mat-icon *ngIf="(stockState.isFetchExpenseItems | async) === false">
          refresh
        </mat-icon>
        <mat-progress-spinner *ngIf="(stockState.isFetchExpenseItems | async)===true" [diameter]="20"
