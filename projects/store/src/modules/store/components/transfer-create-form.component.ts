...
@Component({
  selector: 'app-store-transfer-create-form',
  template: `
    <div class="">
...
export class TransferCreateFormComponent implements OnInit {
  ...
  shops: ShopModel[] = [];
  transfersDatasource: MatTableDataSource<{ quantity: number, product: StoreModel }> = new MatTableDataSource([]);
  transfersTableColumn = ['product', 'quantity', 'subAmount', 'action'];
  selectedProducts: { quantity: number, product: StoreModel }[] = [];
  totalCost = 0;
  ...
  ngOnInit(): void {
    ...
    this.transferFormGroup = this.formBuilder.group({
      ...
      note: ['store transfer', [Validators.required, Validators.nullValidator]],
      ...
      this.dialog.open(InfoDialogComponent, {
        data: {
          message: 'When you submit a transfer will be saved ' +
            'and corresponding storeItems will be updated and you will not be able to edit it again.'
        }
  ...
  removeItem($event: MouseEvent, element: { quantity: number, product: StoreModel }): void {
    $event.preventDefault();
  ...
  updateQuantity(element: { quantity: number, product: StoreModel }, $event: Event): void {
    // @ts-ignore
    ...
    this.transfersDatasource = new MatTableDataSource<{ quantity: number; product: StoreModel }>(this.selectedProducts);
    this.updateTotalCost();
  }
}
